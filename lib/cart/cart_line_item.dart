...
class _CartLineItemState extends State<CartLineItem> {
	...
	Future<void> _removeCartItem() async {
		...
		await context.read<CartModel>().cartLinesRemove(context, widget.lineItem['id']);
		setState(() {
			_cardLoading = false;
		});

		// You can trigger a rebuild of the parent widget or refresh the page here
		// For example, you can call setState in the parent widget, or use a callback to notify the parent widget
		// Alternatively, if you want to refresh a list, you could use a FutureBuilder with a stream of updates
	}
	...
	Widget build(BuildContext context) {
		return Dismissible(
			...
			onDismissed: (direction) async {
				...
				// Trigger a rebuild or smooth refresh
				// For example, using FutureBuilder or setState in a parent widget
			},
								...
								child: Padding(
									...
									child: Column(
										...
										children: [
												...
												children: [
													Flexible(
														child: InkWell(
															...
															child: Text(widget.lineItem['merchandise']['product']['title'],
																	overflow: TextOverflow.ellipsis, style: const TextStyle()),
														),
													),
													Padding(
														padding: const EdgeInsets.only(left: 8),
														child: Text('₹${widget.lineItem['cost']['amountPerQuantity']['amount']}',
											...
											Row(
												children: [
													Text(widget.lineItem['merchandise']['title'],
